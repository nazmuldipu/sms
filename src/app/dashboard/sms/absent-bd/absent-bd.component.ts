...
@Component({
  selector: 'app-absent-bd',
  templateUrl: './absent-bd.component.html',
  styleUrls: ['./absent-bd.component.css']
})

/* 
  1) Get Student List first
*/
export class AbsentBdComponent implements OnInit {
  arr = Array;
  companyId: number;
  classId: number;
  companyList: Company[] = [];
  ...
  send(){
    ...
    this.smsService.sendAbsentBd(generalSMSList)
    .subscribe(
      data => {
        this.message = '';
        data.forEach(resp => {
