...
@Component({
  selector: 'app-result-bd',
  templateUrl: './result-bd.component.html',
  styleUrls: ['./result-bd.component.css']
})
export class ResultBdComponent implements OnInit {
  arr = Array;
  companyId: number;
  classId: number;
  companyList: Company[] = [];
  ...
  send(resultForm) {
    ...
    if (this.generalSMSList.message.length > 0 && this.generalSMSList.number > 0) {
      this.smsService.sendResultBd(this.generalSMSList)
        .subscribe(
          data => {
            this.message = '';
            data.forEach(resp => {
